    ...
    <div class="grid grid-cols-1 md:grid-cols-[3fr_7fr] w-full max-w-7xl px-4 md:overflow-y-auto overflow-y-visible">
      ...
      <div class="message-container mb-8 pt-4 md:border-r-[1px] md:border-gray-300 overflow-y-auto h-[calc(100vh-5rem)] scroll-container">
        ...
        @else
          ...
          <a data-userid="{{$message->_id}}" class="user-message-preview cursor-pointer hover:bg-gray-300 flex items-center grid grid-cols-[2fr_8fr] p-2 mb-2 border rounded-sm items-">
            <div>
              <img class="h-7 w-7 rounded-full border border-black" src="{{ asset('imgs/User/avatarBgremove.png') }}" alt="User Avatar">
            </div>
            <div>
              <p>{{$message->chatPartner}}</p>
            </div>
          </a>
          @endforeach
        @endif
    ...
    <script>
      document.addEventListener('DOMContentLoaded', async function(){
        ...
        messageContainer.addEventListener('click',  function(e){
          ...
          chat[0].messages.forEach(msg => {
            ...
            if(msg.sender === currentUserId){
              warperAppend(warper, 'justify-start', div, timeDiv)
            } else {
              div.classList.add('bg-green-200')
              warperAppend(warper, 'justify-end', timeDiv, div)
            }
        ...
        const markup = data.map(doc => {
          return `<a data-userId="" class="user-message-preview cursor-pointer hover:bg-gray-300 flex items-center grid grid-cols-[2fr_8fr] gap-1 p-2 border rounded-lg items-center">
            <div>
              <img class="h-7 w-7 rounded-full border border-black" src="{{ asset('imgs/User/avatarBgremove.png') }}" alt="User Avatar">
              </div>
              <div>
