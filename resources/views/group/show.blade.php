<x-app-layout>
    ...
    <div class="main-container grid grid-cols-[1fr] md:grid-cols-[3fr_7fr] px-4 md:overflow-y-hidden">
        <div class="mb-2 flex flex-col gap-2 md:border-r md:border-gray-300 order-2 md:order-1 max-h-[calc(100vh-3rem)] md:overflow-y-auto overflow-y-visible scroll-container ">
            @foreach ($posts as $post)
                <a href="{{route('groupPost.show', ['groupPostSlug' => Str::slug($post->title_slug), 'groupSlug' => Str::slug($post->title)])}}" class="cursor-pointer hover:bg-gray-200 p-4 py-4 mt-4 md:bg-inherit bg-[#EAECEF] rounded-[9px]">
                    <div>
                        ...
                        <div>
                            <p class=" text-end text-gray-500 md:text-base md:text-[0.6rem] ml-6"> <b class="text-gray-700"></b></p>
                        </div>
                    </div>                  
                </a>
            @endforeach
        ...
        <div class="flex flex-col items-center justify-center mt-8">
            <h2 class="text-3xl sm:text-4xl md:text-5xl font-serif text-gray-900 text-center relative mb-6">
                <span class="inline-block relative">
                    ...
                    <span class="relative z-10">{{$thema->title}}</span>
                </span>
                <br>
            ...
            <div class="flex items-center justify-center">`
                <img class="md:h-64 md:w-64 h-24 w-24 z-10 "  src="{{asset('imgs/undraw_things-to-say_f5mi.svg')}}" alt="">
            </div>
                <div class="flex items-center gap-2 justify-center mt-8">
                    ...
                    <p class="italic">Neuer Beitrag</p>
                </div>       
            </div>
        </div>
    </div>
    ...
    <div class=" mt-24 md:mt-0">
        <div class=" modal hidden m-8 p-8 md:m-24 md:h-[65%] md:w-[60%] rounded-xl w-[90%] h-[65%] bg-white fixed inset-0 top-[40%] md:top-[30%] left-[45%] transform -translate-x-1/2 -translate-y-1/2  z-50">
            ...
            <form class="grid grid-rows-[2fr_8fr]" method="POST" action="{{route('helping.store')}}">
                @csrf
    ...
    <script>
        ...
        const gemeinden = @json($posts);
        const titles = @json($posts);
